...
class HabitTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(
            name="Тест", email="testuser@example.com", telegram_chat_id=1234567, is_active=True
        )
    ...
    def test_habit_retrieve(self):
        ...
        self.assertEqual(str(Habit.objects.get(user=self.user.pk)), "Жим штанги")
    ...
    def test_habit_validators(self):
        ...
        self.assertEqual(
            response.json()["duration_time"], ["Время выполнения привычки должно быть не более 120 секунд!"]
        )

        data = {"duration_time": 0}
        response = self.client.patch(reverse("habits:habit_update", args=(self.habit.pk,)), data)
        self.assertEqual(response.json()["duration_time"], ["Время выполнения привычки не может равняться нулю!"])

        data = {"periodicity": 10}
        response = self.client.patch(reverse("habits:habit_update", args=(self.habit.pk,)), data)
        self.assertEqual(response.json()["periodicity"], ["Нельзя выполнять привычку реже, чем 1 раз в 7 дней!"])

        data = {"periodicity": 0}
        response = self.client.patch(reverse("habits:habit_update", args=(self.habit.pk,)), data)
        self.assertEqual(
            response.json()["periodicity"], ["Периодичность выполнения не может быть меньше или равна нулю!"]
        )
