...
class UserTestCase(APITestCase):
    ...
    def test_user_update(self):
        field = {"name": "ТестХ",
                 "password": 'test12345'}
        response = self.client.patch(reverse('users:user_update', args=(self.user.pk,)), field)
        data = response.json()
    ...
    def test_user_retrieve(self):
        ...
        self.assertEqual(str(User.objects.get(email="testuser@example.com")), "testuser@example.com")
